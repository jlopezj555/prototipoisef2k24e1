...
public void InsertarDatos(string[] tipos, string[] datos, string tabla)
{
    ...
    catch (Exception ex)
    {
        ...
        {
            Dictionary<string, string> aliasColumnas = new Dictionary<string, string>()
{
    // Tabla Producto
    { "Pk_id_Producto", "ID Producto" },
    ...
    { "estado", "Estado del Producto" },  // Se cambió el alias para diferenciarlo
    { "stock", "Stock Disponible" },
    { "empaque", "Tipo de Empaque" },
    { "Fk_id_receta", "ID Receta" },
    { "comisionInventario", "Comisión de Inventario" },
    { "comisionCosto", "Costo de Comisión" },

    // Tabla Marca
    { "Pk_id_Marca", "ID Marca" },
    { "nombre_Marca", "Nombre de la Marca" },
    { "descripcion", "Descripción" },
    { "fk_id_Producto", "ID Producto Asociado" },
    { "comision", "Comisión de la Marca" } , 
            ...
            // Agregar alias al ComboBox y almacenar el nombre real en el tag de cada ítem
            foreach (string columna in columnas)
            {
                if (aliasColumnas.ContainsKey(columna))
                {
                    var item = new ComboBoxItem(aliasColumnas[columna], columna); // Crea un ítem con alias y nombre real
                    comboBoxCampos.Items.Add(item); // Agregar el ítem al ComboBox
                }
                else
                {
                    var item = new ComboBoxItem(columna, columna); // Agregar solo el nombre real si no hay alias
                    comboBoxCampos.Items.Add(item);
                }
            }

            // Manejar la selección del ComboBox
            comboBoxCampos.SelectedIndexChanged += (sender, e) =>
            {
                var selectedItem = comboBoxCampos.SelectedItem as ComboBoxItem;
                if (selectedItem != null)
                {
                    string nombreReal = selectedItem.RealColumnName;

                    // Ahora puedes usar 'nombreReal' para la consulta SQL
                }
            };
        }

        // Clase para almacenar alias y nombre real de las columnas
        public class ComboBoxItem
        {
            public string Alias { get; set; }
            public string RealColumnName { get; set; }

            public ComboBoxItem(string alias, string realColumnName)
            {
                Alias = alias;
            ...
            {
                return Alias; // Muestra solo el alias en el ComboBox
            }
        }
